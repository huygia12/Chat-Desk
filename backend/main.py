    ...
    <div class="card">
      ...
      <table>
        ...
        <tr><td>LLM_PROVIDER</td><td><code>{settings.LLM_PROVIDER}</code></td>
            <td class="{'ok' if settings.LLM_PROVIDER.strip().lower() in ('groq', 'openai', 'gemini') else 'err'}">{'Set' if settings.LLM_PROVIDER.strip().lower() in ('groq', 'openai', 'gemini') else 'INVALID'}</td></tr>
        ...
        <tr><td>GEMINI_API_KEY</td><td><code>{'***' + settings.GEMINI_API_KEY[-4:] if settings.GEMINI_API_KEY else ''}</code></td>
            <td class="{'ok' if settings.GEMINI_API_KEY else 'warn'}">{'Set' if settings.GEMINI_API_KEY else 'Not set'}</td></tr>
